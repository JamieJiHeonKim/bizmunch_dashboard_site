...
function DELETE_MANAGER(props) {
  const [loading, setlaoding] = useState(false);

  async function handlepost() {
    try {
      setlaoding(true);
      const res = await Admin_Delete_Company(props.manager._id);
      if (res.status === 200) {
  ...
  return (
    <dialog id="Delete_Manager" className="modal">
      <div className="modal-box p-12">
        <MODAL_HEADER heading={`Delete '${props.manager.name}'`} />
